...
var wordsString = `a -- неопределенный артикль
...
var arr = wordsString.split('\n');

arr.forEach( (item, index) => {
  let newItem =  "\"" + item + "\"";
  // arr[index] = newItem;
  let tempArr = newItem.split('--');
  // let obj = {
  //   eng: tempArr[0],
  //   ru: tempArr[1],
  // }
  arr[index] = `${ tempArr[0].trim() }":"${ tempArr[1].trim() }`;
})

var output = `let wordsJSON = '{ ${arr} }'`;


const fs = require('fs');


fs.writeFile('test.js', output, (err) => {
  if(err) throw err;
